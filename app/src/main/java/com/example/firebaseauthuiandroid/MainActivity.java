...
public void logOut() {
    ...
    startActivity(i);
      ...
      public void listingFirestore(){
          db.collection("cities").addSnapshotListener(new EventListener<QuerySnapshot>() {
              @Override
              public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                  if (error != null){
                      Toast.makeText(MainActivity.this, "not reading from Firestore", Toast.LENGTH_LONG).show();
                  }
                  List<String> cities = new ArrayList<>();
                  for(DocumentSnapshot doc : value.getDocuments()){
                      String cityValue = doc.getString("name");
                      cities.add(cityValue);
                  }
                  arrayAdapterCity = new ArrayAdapter<String>(MainActivity.this, R.layout.item_list, R.id.listView, cities);
                  textViewCities = findViewById(R.id.listViewFirestore);

                  textViewCities.setAdapter(arrayAdapterCity);
